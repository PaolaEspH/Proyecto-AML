...
* Montos mayores o iguales al umbral por segmento: 500000 para cuentas personales, 1000000 para cuentas de PYME y PEP, 10000000 para corporativos.
```{r}
umbral_pep     <- 1000000   
...
alertas <- transacciones %>%
  ...
  mutate(
    ...
    umbral_segmento = case_when(
      segmento == "Personal"    ~ 500000,
      segmento == "PYME"        ~ 1000000,
      segmento == "Corporativo" ~ 10000000,
      TRUE                      ~ 500000   # No asignado, se trata como personal
    ),
    # Reglas
    ...
    num_reglas       = regla_monto_alto + regla_pep + regla_frecuencia,
    alerta           = num_reglas > 0,
...
transacciones_aml <- alertas %>%
  select(
    ...
    trx_diarias, monto_diario,
    alerta, id_regla_principal
  )
write.csv(transacciones_aml, file = '../res/alertas.csv')
