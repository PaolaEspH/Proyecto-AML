...
clientes <- tibble(
  ...
  mutate(
    es_pep = if_else(
      segmento == "Personal",
      rbinom(n(), size = 1, prob = 0.03),
      0
    ),
    # Ingreso base distinto por segmento
    ingreso_base = case_when(
      segmento == "Personal"    ~ rlnorm(n(), meanlog = log(600000),  sdlog = 0.6),
      segmento == "PYME"        ~ rlnorm(n(), meanlog = log(1200000), sdlog = 0.6),
      segmento == "Corporativo" ~ rlnorm(n(), meanlog = log(4000000), sdlog = 0.7)
    ),
    # Factor PEP
    factor_pep = if_else(es_pep == 1, 1.8, 1.0),
    ingreso_mensual = round(ingreso_base * factor_pep, 0)
  ) %>%
  select(-ingreso_base, -factor_pep)
...
transacciones <- tibble(
  ...
  mutate( #montos coherentes
    monto = rlnorm(n(), log(ingreso_mensual/10), 0.5)) %>%
